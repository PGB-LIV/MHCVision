...
def convert_score_for_beta(file):
    ...
    percent_neg = len(neg_data) / len(template)
    w1_i = round((1 - percent_neg), 2)
    ...
    a1i = phi1i*mean1i
    b1i = phi1i+(1-mean1i)
    a1fix[0], a1fix[1] = a1i-(a1i/2), a1i+(a1i/2)
    b1fix[0], b1fix[1] = b1i-(b1i/2), b1i+(b1i/2)
...
class BMM:
    ...
    def termination(self):
        ...
        while kq > tq:
            ...
            if round(c_w1[-1], str_size - 1) == 1 and percent_neg != 0:
                beta1_fix = True
...
if '-h' in argv or '--help' in argv or check_valid_argument(argv) == True:
    ...
    # if all argument values are correct, make the estimation
        ...
        if model.checking_estimated_parameters() == True:
            size1, size2, _ = model.checking_estimated_parameters()
            if size1 == 1:
                print('Done! The input data is estimated to all binding peptide, cannot operate FDRs calculation.\n')
            if size2 == 1:
                print('Done! The input data is estimated to all non-binding peptide, cannot operate FDRs calculation.\n')
        else:
